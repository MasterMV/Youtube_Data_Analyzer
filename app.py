...
def video_comments():
    ...
    numeric_data = comment_sentiment2.select_dtypes(include=[np.number])

    # Check if 'like_count' column is present in the numeric data
    if 'like_count' in numeric_data.columns:
        like_count_sentiment_corr = round(numeric_data.corr().loc['like_count', 'compound'], 2)
    else:
        like_count_sentiment_corr = None  # Handle case where 'like_count' might not be available
